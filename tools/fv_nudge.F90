...
module fv_nwp_nudge_mod
 ...
 use constants_mod,     only: pi=>pi_8, grav, rdgas, RAD_TO_DEG, cp_air, kappa, cnst_radius =>radius
...
! <--- h1g, 2012-10-22
 ...
 logical :: do_q_bias  = .false.
 logical :: do_t_bias  = .false.
...
!------------------------------------------------------------------------------------------
  integer :: id_ht_err
!!--------x1y add diag variables for nudging increments
  integer :: id_u_adj, id_v_adj, id_t_adj, id_q_adj, id_q_adj_bias, id_t_adj_bias
...
! local:
   ...
   logical used
!! x1y add the prof_lat
   real :: prof_lat(is:ie,js:je)
   real:: lat_tmp
...
!$OMP parallel do default(none) shared(npz,press,ak,bk,P_relax,P_norelax,profile)
  ...
  ! above P_norelax, no nudging. added by h1g
   ...
   enddo
   profile(1) = 0.
!  simple smooth for skip_bot by x1y
   if ( kbot_winds > 0 ) then
     profile(npz-kbot_winds-1) = 0.5
     profile(npz-kbot_winds) = 0.25
     do k = npz-kbot_winds+1, npz
       profile(k) = 0.0
     enddo
   endif
...
!$OMP parallel do default(none) shared(npz,press,P_relax,P_norelax,prof_t)
     do k=1,npz
       if ( press(k) < P_relax ) then
         prof_t(k) =  max(0.01, press(k)/P_relax)
       endif
     ...
     ! simple smooth for skip_bot by x1y
     if ( kbot_t > 0 ) then
       prof_t(npz-kbot_t-1) = 0.5
       prof_t(npz-kbot_t) = 0.25
       do k = npz-kbot_t+1, npz
         prof_t(k) = 0.0
       enddo
     endif
   else
...
!$OMP parallel do default(none) shared(npz,press,P_norelax,prof_q)
     ...
     ! simple smooth for skip_bot by x1y
     if ( kbot_q > 0 ) then
       prof_q(npz-kbot_q-1) = 0.5
       prof_q(npz-kbot_q) = 0.25
       do k = npz-kbot_q+1, npz
         prof_q(k) = 0.0
       enddo
     endif
   else
...
! *t_obs* is virtual temperature
   ...
   prof_lat(:,:) = 1.0
   if ( q_lat_varying ) then
     do j=js,je
       do i=is,ie
         lat_tmp = agrid(i,j,2)*RAD_TO_DEG
         if ( abs(lat_tmp) < 35.0 ) then
           prof_lat(i,j) = 1.0
         else if ( abs(lat_tmp) > 45.0 ) then
           prof_lat(i,j) = 0.
         else
           prof_lat(i,j) = 1.0 - (abs(lat_tmp) - 35.0)/10.0
         endif
       enddo
     enddo
   endif
...
!$OMP                                  q,pt,rdt,ps_fac,climate_nudging)
   ...
   if ( id_t_adj > 0 ) used=send_data(id_t_adj, t_dt(is:ie,js:je,:), Time)
   !! bias correction
   if( do_t_bias ) call temp_bias_correction( t_dt, is, ie, js, je, isd, ied, jsd, jed, area,npz)
   if ( id_t_adj_bias > 0 ) used=send_data(id_t_adj_bias, t_dt(is:ie,js:je,:), Time)
...
!
 ...
 subroutine temp_bias_correction ( t_dt, is, ie, js, je, isd, ied, jsd, jed, area, nk )
   integer, intent(IN) :: is,  ie,  js, je
   integer, intent(IN) :: isd, ied, jsd,jed
   integer, intent(IN) :: nk
   real, intent(inout):: t_dt(is:ie,js:je,1:nk)
   real, intent(IN), dimension(isd:ied,jsd:jed) :: area
!
   real:: esl, total_area
   real:: bias, psum
...
!  esl = 0.000001       ! K
   ...
   do k = 1,nk
     bias = g0_sum(t_dt(:,:,k), is, ie, js, je, 1, .true., isd, ied, jsd, jed, area)

     if ( abs(bias) < esl ) then
       if(master .and. nudge_debug .and. k > 30) write(*,*) 'Very small temp bias=', -bias, ' No bais adjustment'
       return
     else
       if(master .and. nudge_debug .and. k > 30)  write(*,*) 'Significant temp bias=', -bias
     endif
     ...
     if ( bias > 0. ) then
       ...
       do j=js,je
         do i=is,ie
           if ( t_dt(i,j,k) > 0.0 ) then
             t_dt(i,j,k) = max(0.0, t_dt(i,j,k) - bias)
           endif
         enddo
       enddo
     else
       ...
       do j=js,je
         do i=is,ie
           if ( t_dt(i,j,k) < 0.0 ) then
             t_dt(i,j,k) = min(0.0, t_dt(i,j,k) - bias)
           endif
         enddo
       enddo
     endif
   ...
   enddo  !! loop over k
 end subroutine temp_bias_correction
